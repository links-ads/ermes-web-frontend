...
const RangeDateTimePicker: React.FC<{ editState; dispatchEditAction; disabledRangePickerTime?; disabledBeforeToday? }> = (
  props
) => {
  ...
  const { editState, dispatchEditAction, disabledRangePickerTime, disabledBeforeToday } = props as any
  ...
  const disableDateRange = (current) => {
    // TODO: check if this is the correct way to disable dates
    // Can not select days after start date + 30 days
    const start = moment(startDate)
    const maxEndDate = start.add(30, 'days')
    const diff = current.diff(maxEndDate, 'days')
    // if (diff > 0) {
    //   return current && current > current.add(diff, 'days')
    // }
    return current && current > current.add(diff, 'days') //current && current > moment().endOf('day')
  }
      ...
      <Grid item>
        ...
        <LocaleProvider locale={locale}>
          <RangePicker
            ...
            disabledDate={disabledBeforeToday ? disabledDate : undefined}
            disabledTime={disabledRangeTime}
            onChange={updateRangeDate}
            showTime={
              !disabledRangePickerTime
                ? {
                    defaultValue: [
            ...
            allowClear
            format={!disabledRangePickerTime ? 'ddd DD MMMM YYYY - HH:mm' : 'ddd DD MMMM YYYY'}
            style={{ width: 560 }}
            locale={locale}
          />
        </LocaleProvider>
