...
import { CoordinatorType, EditActionType, EditStateType, OperationType } from './map-dialog.hooks';
import { ProvisionalFeatureType } from './map.contest';
import TextField from '@material-ui/core/TextField';
import { FormControl, FormHelperText, Grid, IconButton, InputLabel, MenuItem, Select } from '@material-ui/core'
            ...
            {itemType == 'Mission' && (
                <Grid style={{ marginBottom: 8 }}>
                    <TextField
                        ...
                        helperText={(editError && editState.title.length === 0) ? t("maps:mandatory_field") : `${editState.title.length}/255`}
                    ...
                    <Grid container>
                        <Grid item style={{ flex: 1 }}>
                            <FormControl margin='normal' style={{ minWidth: '50%' }} disabled={Object.entries(orgOptions).length < 2} error={editError && editState.coordinatorType === CoordinatorType.NONE}>
                                ...
                                <FormHelperText>{editError && editState.coordinatorType === CoordinatorType.NONE && t("maps:mandatory_field")}</FormHelperText>
                            </FormControl>
                        </Grid>
                        <Grid item style={{ flex: 1 }}>
                            <FormControl margin='normal' style={{ minWidth: '50%' }} disabled={Object.entries(teamsOptions).length === 0}>
                        ...
                        <Grid item style={{ flex: 1 }}>
                            <FormControl margin='normal' style={{ minWidth: '50%' }} disabled={Object.entries(usersOptions).length === 0}>
