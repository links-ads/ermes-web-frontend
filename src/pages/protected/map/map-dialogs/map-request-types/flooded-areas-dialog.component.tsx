...
export function FloodedAreasDialog({
  operationType,
  editState,
  dispatchEditAction,
  editError
  ...
  const layersApiFactory = useMemo(
    () => LayersApiFactory(backendAPIConfig, backendUrl, axiosInstance),
    [backendAPIConfig]
  )
  ...
  const dataTypeOptions = useMemo(() => {
    if (Object.entries(apiHandlerState.result).length === 0) return []
      ...
      apiHandlerState.result.data.layerGroups
        .filter((l) => l.groupKey === 'flooded area map')
        .forEach((group) => {
