...
export function Missions({ missions }) {
  const { t } = useTranslation()

  const data = useMemo(() => {
    if (!missions) return []

    const data = missions.map((p) => ({
      id: t(`labels:${p.id.toLowerCase()}`),
      value: p.value,
      valueColor: EmergencyColorMap.Mission
    }))
  ...
  return (
    <div>
      <Typography variant="body2">{t('labels:chart_missions')}</Typography>
