...
const colors = Object.entries(CulturalColorMap).flat()
...
export const categories: BeniType[] = Object.keys(CulturalColorMap) as BeniType[]

// Define the type of a counter in a cluster
type CounterType = {
  [k in BeniType]?: mapboxgl.Expression
}

// Expression that accumulates the count of elements in a cluster per each "type"
// TODO type and category refer both to "type of feature". Stick to 1 single definition
export const culturalClusterProperties = categories.reduce<CounterType>(
 ...
 */
export function updateCulturalMarkers(
  sourceName: string,
  markersRef: React.MutableRefObject<[object, object]>, //= [ allmarkers, onscreen]
  map: mapboxgl.Map
) {
  const relevantKeys = categories
  const colors = Object.values(CulturalColorMap)
  return updateMarkers<BeniType>(sourceName, relevantKeys, colors, markersRef, map)
}
